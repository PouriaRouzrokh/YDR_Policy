...
def main(config: SimpleNamespace = None, logger: logging.Logger = None):
    ...
    if logger is None:
        logger = DataCollectionLogger(
            name="scrape", 
            level=logging.INFO, 
            path=config.LOGGING.SCRAPER_LOG_FILE
        )
    ...
    logger.info(f"  - Input directory: {config.PATHS.RAW_DATA_DIR}")
    ...
    logger.info(f"  - Scraping LLM model: {config.LLM.SCRAPER_LLM_MODEL}")
    ...
    df_with_policies = scrape_policies(
        original_df, 
        base_path=config.PATHS.RAW_DATA_DIR,
        config=config,
        logger=logger
    )
    
    # Save the scraped policies
    output_path = os.path.join(config.PATHS.PROCESSED_DATA_DIR, "scraped_policies_data.csv")
